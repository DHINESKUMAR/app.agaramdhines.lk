...
export default function Fees() {
  ...
  if (view === "status") {
    ...
    const exportToPDF = () => {
      ...
      const tableData = [
        ...paidStudents.map(s => [s.name, s.student_id || s.id, s.grade, "Paid"]),
        ...outstandingStudents.map(s => [s.name, s.student_id || s.id, s.grade, "Unpaid"])
      ];

      autoTable(doc, {
        startY: 20,
        head: [['Student Name', 'Student ID', 'Grade', 'Status']],
        body: tableData,
        theme: 'grid',
        styles: { fontSize: 10 },
                ...
                outstandingStudents.map(student => (
                  <div key={student.id} className="flex justify-between items-center border-b border-gray-100 pb-2 last:border-0 last:pb-0">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{student.student_id || student.id} | {student.grade}</p>
                    </div>
